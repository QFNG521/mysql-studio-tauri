...
| Windows | `.msi` / `.exe` | x64，Windows 10+ |
...
| Windows | `tauri.windows.conf.json` | `.msi`、`.exe`（NSIS） |
...
NSIS 与 AppImage 打包依赖外部工具，工作流里做了失败回退（Windows 退为仅 MSI，Linux 退为 deb + rpm），保证发版不会因为单一格式失败而中断。
